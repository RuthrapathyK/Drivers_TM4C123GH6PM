...
void TIM_Init(void)
{
    ...
    REG_WRITE(TIMER0->CFG, TIM_NonConcatenatedMode, 0, 3);  // Select 16-bit Timer without concatenation
    REG_WRITE(TIMER0->TAMR, TIM_Periodic, 0, 2); // Periodic mode
    REG_WRITE(TIMER0->TAMR, TIM_CountDown, 4, 1); // Downcount mode
    REG_WRITE(TIMER0->TAMR, TIM_ReloadSameCycle, 8, 1); //Reload values upon timeout not on next cycle

    /* Configure Prescalar */
    REG_WRITE(TIMER0->TAPR, 0, 0, 8);

    /* Load Start Value to start counting */
    REG_WRITE(TIMER0->TAILR, 159, 0, 16); // Load the value to generate 10us us Once interrupt

    /* Configure Interrupt Mask - Disable Interrupt */
